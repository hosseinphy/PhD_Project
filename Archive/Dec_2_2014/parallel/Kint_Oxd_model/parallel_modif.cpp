...
int main (int argc, char ** argv){
 ...
 sprintf(filename2,"frontname%0.5f%0.6f_%0.3f.txt",t_step,To,Ctshold); 
...
if(myid==0){
 for(int i=0;i<Nxp;i++){
  wrt<<i<<"\n";
  for(int j=0;j<Nyp;j++){
   wrt<<j<<"\t"<<ptemp[i][j]<<"\t"<<pconc[i][j]<<"\t"<<pgrad[i][j]<<"\n";
  }
    wrt<<"\n\n";
 }
}
...
void oxidation() {
 ...
 ////// Updating the temprature////////
     ...
     for(i=1;i<Nxp-1;i++){     
        for (j=1;j<Nyp-1;j++){
             ptemp[i][j]=ptemp[i][j]+(t_step*(pgrad[i][j]+(1./tou_c)*alphap[i][j]*pconc[i][j]));
        }
     }
  }
...
/***** temperature write function ****/
 ...
 int a,b;
     ...
     if (t%printfreq == 0) {			
        sprintf(filename6, "temp_field%d_%d_%0.5f_%0.5f.dat",f_n,t/printfreq,t_step,To);
