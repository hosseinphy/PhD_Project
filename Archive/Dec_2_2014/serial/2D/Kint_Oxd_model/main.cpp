           ...
           for (j=1;j<=Ny;j++){ //here I excluded the first (j=1) and last (j=Nyp-1) horizental planes
               ...
               if(tc[i][j]>=0){
                    ...
                    if(T[i][j]>=Tg){
                            ...
                            if(tc[i][j]== 0.0){// those reactant who has a time equall zero is a possible reactant for buring
                       
                                  tc[i][j]=(t*t_step)+tou_c;//end time of burning
                                  alpha[i][j]=1;
                            }

                            else if(tc[i][j]>(t*t_step)){
...
void front(){
    ...
    /*    //Writing temp profile

          sprintf(filename4,"profile%d.txt",t/printfrontpos);
           FILE * frontemp = fopen(filename4, "w");
             fprintf(frontemp, "Average_position , Time\n");
             fprintf(frontemp, "\n%f, %f\n",ave_pos , t*t_step);
             fprintf(frontemp, "\n Positions\n\n");
               for (j=0;j<Ny;j++){
                 fprintf(frontemp, "%d, %f ,%f\n", j, max_temp_pos[j], max_temp[j]);
               }
           fclose(frontemp);
   */
