...
/**********************************/
 ...
 int main (int argc, char ** argv){
  ...
  //creat file for storing front position
   sprintf(filename2,"frontoxd%d_%0.3f_%0.3f.txt",f_n,tou_c,Tg);
...
void Calc_higgins(){
 ...
 //Updating tempreture afterwards
        ...
        for(i=1;i<=Nx;i++){    
         T[i]=T[i]+(t_step*(Grad[i]+(1/tou_c)*alpha[i]*C[i]-cap_Gamma*(T[i]-To)));
       }
 }
...
void Temp_write(){
   ...
   if (t%printfreq == 0) {			
       sprintf(filename5, "array_oxd%0.5f%d.txt",t_step,t/printfreq);
