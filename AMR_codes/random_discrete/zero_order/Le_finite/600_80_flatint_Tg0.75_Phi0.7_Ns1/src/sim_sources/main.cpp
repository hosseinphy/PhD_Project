...
/*
    ...
    { 
        ...
        FILE *fp;
        int restart_fn=1400000 ;

        string fname= "aaa";
        string fext= ".dat";

        stringstream restart_time;
        restart_time<<restart_fn;
        string Full_Filename;

        Full_Filename=fname + restart_time + fext;

        std::ifstream infile(Full_Filename.c_str());
        //std::cout<<Full_Filename.c_str()<<std::endl;
        	if (!infile) std::cerr << "Could not open the file!" << std::endl;
        	else
		{
        		infile >>nodesize>> ;	
			...
			//// Position and size of the particles: 
        		double *rest_x = new double[nodesize];
        		double *rest_y = new double[nodesize];
        		double *rest_tempr = new double[nodesize];
        		double *rest_conc = new double[nodesize];
	        	...
	        	#pragma omp parallel default(shared) private(t) num_threads(numprocs)
        		{
                	#pragma omp for schedule(static)
                		for(t=0;t<nodesize;t++)
               	 		{
                			infile>> rest_x[t] >> rest_y[t] >> rest_tempr[t]>> rest_conc[t] ;
         			}
       			}			
		}
    }
