...
double particle_loop(Random& r, int ii, int jj, int N) 
{
   int 	   idx ;
   VecR    Rold,Rnew ;
   double  w =0.0;
	for (idx = ii * jj; idx < (ii + 1 ) * jj; ++idx)
	{
		//std::cout<<ii<<" \t "<<idx<<std::endl;	
		VCopy(Rold, part[idx].r) ; 
		...
		if(!Overlap_test(Rnew.x,Rnew.y,idx,N))
       		{
			VCopy(part[idx].r, Rnew) ;			
		  	w = w + 1. ;						//Accepted moves in one step
 		}		
	}		
return w ;								// Total number of moves (including those result in overlap
}
...
int main(int argc, char* argv[])
{
  ...
  kk = 100;                  // Number of tasks
  ll = 4;                    // The leapfrogging stride
...
//write final shifted config
  ...
  clock_t end =clock();			//clock tick ends	

  double elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;

  std::cout<<"Elapsed time for the simulation is	: "<<elapsed_secs<<std::endl;
...
void SingleStep()
{
	...
	//std::vector<double> Acc_mov(kk, std::numeric_limits<double>::quiet_NaN());
	std::vector<double> Acc_mov(kk);
/*
	#if HAVE_OPENMP
	#pragma omp parallel for
	#endif	
*/	//begin openmp
	#pragma omp parallel default(shared) private(ii,jj) num_threads(numprocs)
	{
 		#pragma omp for 			
		...
		//LOOP OVER PARTICLES
	 	for (ii = 0 ; ii< kk; ii++)
	 	{
        		jj = (N * (ii + 1))/kk - (N * ii)/kk ;						// devide number of particle by an arbit. chunck width (kk)	
			// for each processor id ii :
			...
			Acc_mov[ii] = particle_loop(r,ii,jj,N);
      		} //.............................................
		// ** LOOP OVER P ARTICLES COMPLETE
	}// end openmp

  	// Sum up the accepted moves (weights) from the individual tasks
  	Acmmva = Acmmva + accumulate(Acc_mov.begin(), Acc_mov.end(), 0.0);
 	Acm = Acm + N ;	
	...
	if ( (stepCount % Iprint) == 0 ) 
 	{
	 	...
	 	sprintf(filename2, "Partconf_real_Lx_Ly_%d_%d_%d_Fract%.1f_Ns%d_Dim%0.3f.dat",input.boxY, input.boxX, stepCount, Fract, (int)Density,Dim);
...
void SetParams()
{
 ...
 //Np = VProd (initUcell);
 rCut = 4.  ;//1. - .5 * Delr ;					//max rCut is a radius of a circle centered about particle above which there is no interaction 
 ...
 Maxbin.x = (int)(((region.x * Side / 2.) - 1.) / Delr) ;		// Maxbin (n-1) = (Rij - (rCut(t=0) + "\Delta r" /2)) / Delta r									        								    // where we assume the rCut(t=0) is equal to Dim - "\Delta r" /
